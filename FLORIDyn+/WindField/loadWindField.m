function [U, I, UF, Sim] = loadWindField(fieldScenario,varargin)
    ...
    % Wind field data
    windSpeed       = 8;        % m/s
    windAngle_start = 20;       % Degree, will be converted in rad
    windAngle_end   = 45;
    ChangeTime      = 300;      % time at which the wind starts rotating
    Change_Duration = 200;      % how long rotation should last
    ambTurbulence   = .06;      % in percent
    posMeasFactor   = 5000;     % Determines location of measurement points
    ...
    U.windSpeed     = windSpeed; %zelf toegevoegd, dit is de ingegeven windsnelheid

    function v = generate_ou_wind(NoTimeSteps, TimeStep, windSpeed)
        mu_abs = windSpeed; % mean value
        theta = 0.05; % mean reversion rate
        sigma_abs = sqrt(2*theta) * 0.045;
        dt = TimeStep; % time step in seconds
        
        v = zeros(NoTimeSteps, 1);    % Pre-allocate wind speed array
        v(1) = mu_abs;          % Set initial value
    
        for t = 2:NoTimeSteps
            dW = randn * sqrt(dt); % Gaussian random variable
            v(t) = v(t-1) + theta * (mu_abs - v(t-1)) * dt + sigma_abs * dW;
        end
    end

    function angle = generate_ou_windang(NoTimeSteps, TimeStep, windAngle_start)
        mu_dir = windAngle_start/180*pi; % mean value
        theta = 0.02; % mean reversion rate
        sigma_dir = sqrt(2*theta)*2.64/180*pi; % standard deviation	
        dt = TimeStep; % time step in seconds

        angle = zeros(NoTimeSteps, 1);    % Pre-allocate wind speed array
        angle(1) = mu_dir;          % Set initial value
    
        for t = 2:NoTimeSteps
            dW = randn * sqrt(dt); % Gaussian random variable
            angle(t) = angle(t-1) + theta * (mu_dir - angle(t-1)) * dt + sigma_dir * dW;
        end
    end
    ...
    switch fieldScenario
        case 'const'
            ...
            U.ang = ones(1,measPoints)*windAngle_start/180*pi;
        ...
        case '+60DegChange'
            ...
            U.ang = ones(NoTimeSteps,measPoints).*windAngle_start/180*pi;
            startI = round(ChangeTime/TimeStep);
            changeAng = linspace(windAngle_start/180*pi,windAngle_end/180*pi,startI);
    ...
    %             U.ang(2*startI+3*offset+1:end,4) + changeAng(end);
        ...
        case 'Custom'
            ...
            Change_Rate = round(Change_Duration/TimeStep);
            Timepoint = round(ChangeTime/TimeStep);
            changeAng = linspace(0,(windAngle_end-windAngle_start)/180*pi,Change_Rate);
    
            
    
            U.ang(Timepoint+1: Timepoint + Change_Rate, :) = U.ang(Timepoint+1: Timepoint+Change_Rate ,:) + changeAng';
            U.ang(Timepoint+Change_Rate+1:end,:) = U.ang(Timepoint+Change_Rate+1:end,:) + changeAng(end);
    
    
            %Throw error if the simulation time is set too short
            if (Timepoint+Change_Rate+10)>NoTimeSteps
                error(['simulation is too short, set SimDuration'...
                    ' at least to ' num2str((Timepoint+Change_Rate+10)*TimeStep) 's.'] ...
                    )
            end
            %Throw error is start angle is bigger than end
            if windAngle_start>windAngle_end
                error(['Start angle should be smaller than end angle'] ...
                    )
            end
            % Constant ambient turbulence
            I.val = ones(1,measPoints)*ambTurbulence;
        ...
        case 'Normal_distribution'
            % Creating a windspeed vector with a normal distribution
            U.abs = ones(NoTimeSteps,measPoints);
            for i = 1:length(posMeas)
                v = generate_ou_wind(NoTimeSteps, TimeStep, windSpeed);
                U.abs(:,i) = U.abs(:,i).*v;
            end
            
            % Changing angle: [t x m] matrix
            U.ang = ones(NoTimeSteps,measPoints);
            for i = 1:length(posMeas)
                angle = generate_ou_wind(NoTimeSteps, TimeStep, windAngle_start);
                U.ang(:,i) = U.ang(:,i).*angle;
            end
