...
for i_T = 1:length(T.D)
    ...
    labels{end-nT+i_T} = ['T' num2str(i_T)];
    text(T.pos(i_T,1) - 100, T.pos(i_T,2), T.pos(i_T,3) + 350, labels{i_T}, ...
   'FontSize', 15, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'backgroundcolor', 'white');
end
...
if Vis.Snapshots
    ...
    NoTimeSteps = length(Sim.TimeSteps);
    if k == 400
        print('./Snapshot/start', '-dpng', '-r300');
    elseif k == 470
        print('./Snapshot/mid', '-dpng', '-r300');
    elseif k == 700
        print('./Snapshot/end', '-dpng', '-r300');
    end
end